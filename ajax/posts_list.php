...
$posts = mysql_query("SELECT SQL_CALC_FOUND_ROWS posts.*, users.name as author_name FROM posts JOIN users ON posts.author_id = users.user_id WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT ". $page * $show .", {$show}");
